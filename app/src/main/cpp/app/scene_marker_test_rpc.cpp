...
public:
    ...
    int Update(AppData &appData,SceneData &sceneData,FrameDataPtr frameDataPtr){
        ...
        {
            //====================== 检测画面中中的marker和棋盘格，仅作调试，需要删除 ===============================
            cv::Mat chess_image=frameDataPtr->image.front(); //相机图像;
            cv::Mat aruco_image=chess_image;
            std::vector<cv::Point2f> imagePoints;
            if(cv::findChessboardCorners(chess_image,cv::Size(9,6),imagePoints)){//,cv::CALIB_CB_ADAPTIVE_THRESH|cv::CALIB_CB_FAST_CHECK|cv::CALIB_CB_NORMALIZE_IMAGE)){
                cv::cvtColor(chess_image, chess_image,cv::COLOR_GRAY2BGR);
                cv::drawChessboardCorners(chess_image, cv::Size(9,6), imagePoints,true);
            }
            else cv::cvtColor(chess_image, chess_image,cv::COLOR_GRAY2BGR);
            ...
            detector.detectMarkers(aruco_image, markerCorners, markerIds, rejectedCandidates);
            if(!markerIds.empty()){
//                cv::cvtColor(aruco_image,aruco_image,cv::COLOR_GRAY2BGR);
                cv::aruco::drawDetectedMarkers(chess_image, markerCorners, markerIds);
            }
//            else cv::cvtColor(aruco_image,aruco_image,cv::COLOR_GRAY2BGR);
            ...
            SerilizedObjs cmdSend={
                    ...
                    {"chessboard_image",chess_image}, //仅作调试，需要删除
            };
