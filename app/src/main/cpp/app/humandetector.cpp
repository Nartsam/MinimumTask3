...
#include"rudesocket-1.2.0/socket.h"


_FF_BEG template<typename CharT>
inline bool IsDirChar(CharT ch){   //提供的代码中找不到IsDirChar的实现，这里推测用法自行实现了此函数
...
using cv::Mat;
enum{
    nct_unknown=0,nct_char,nct_uchar,nct_short,nct_ushort,nct_int32,nct_int64,nct_float,nct_double,nct_string,nct_image,nct_file,nct_list,
};
struct nct{
    ...
    public:
        std::string ext;
    public:
        Image(const cv::Mat &img,const std::string &_ext=".jpg"):cv::Mat(img),ext(_ext){
        }
    };
    class File{
    public:
...
public:
    static int32 nct_to_cv(int32 type){
        CV_Assert(type!=nct_int64);
        return type==nct_char?CV_8S:type==nct_uchar?CV_8U:type==nct_short?CV_16S:type==nct_ushort?CV_16U:type==nct_int32?CV_32S:type==nct_float?CV_32F:type==nct_double?CV_64F:0;
    }
    static int32 cv_to_nct(int32 cvt){
        return cvt==CV_8S?nct_char:cvt==CV_8U?nct_uchar:cvt==CV_16S?nct_short:cvt==CV_16U?nct_ushort:cvt==CV_32S?nct_int32:cvt==CV_32F?nct_float:cvt==CV_64F?nct_double:nct_unknown;
    }
private:
...
private:
    ...
    void _put(const nct::Image &m){
        this->_put_x([&m,this](_ObjHead &head){
            ...
            if(!cv::imencode(m.ext,m,buf))
                throw std::runtime_error("imencode failed"); //std::exception不支持字符串构造，这里修改了用法
    ...
    void _put(const std::vector<nct::Image> &v){
        _put_list(v);
    }
    template<typename _ValT>
...
class NetObjs: public std::map<std::string,ObjStream>{
    typedef std::map<std::string,ObjStream> _BaseT;
public:
    using _BaseT::_BaseT;
    bool hasError(){
        auto itr=this->find("error");
        if(itr==this->end())
            return false;
...
class _BFC_API NetcallServer: public rude::Socket{
...
HumanDetector &HumanDetector::Global(){
    static HumanDetector _inst;
    return _inst;
}
static int count=0;
bool HumanDetector::detect(const cv::Mat &image){
    if(!mCameraMatValid.load()) return false;
    if(image.empty()){
        warnf("Detect Failed: Detect image is empty")
        return false;
    }
    if(!mServerStarted.load()){
        warnf("Detect Failed: Server is not running")
        return false;
    }
    if(mImageProcessed.load()){
        mImageMutex.lock();
        if(++count>100){
            mImage=image.clone();
            mImageProcessed.store(false);
        }
...
bool HumanDetector::camera_mat_valid() const{
    return mCameraMatValid.load();
}

void HumanDetector::set_camera_mat(const cv::Matx33f &camera_mat){
    mImageMutex.lock();
    mCameraMat=camera_mat;
    mCameraMatValid.store(true);
    mImageMutex.unlock();
}

HumanDetector::HumanDetector(){
    mCubeRender=std::make_unique<CubeRender>();
    mCubeRender->initialize();
//    TestModel=Model::GetModelFromLocalFile("TestModel",Config::AppDataDir+("/Model/RM/axle/axle.obj"));
...
bool HumanDetector::start_server(const std::string &ip,int port){
    if(mServerStarted.load()){
        infof("Detect Server Has Already Started.")
        return false;
    }
    if(mServerIsStarting.load()) return false;
    mServerIsStarting.store(true);
    mWorker=std::thread(&HumanDetector::thread_main,this,ip,port);
    return true;
}
bool HumanDetector::server_started()const{
    return mServerStarted.load();
}

void HumanDetector::render(const glm::mat4 &project,const glm::mat4 &view){
    mResultMutex.lock();
    mCubeRender->initialize();
...
//            cv::Point3f(-0.07040677f, -1.05731999f, 1.5701225f),
//            cv::Point3f(-0.0238807f, -1.10950939f, 1.58218852f),
//            cv::Point3f(-0.12337772f, -1.09849473f, 1.58684821f),
//            cv::Point3f(0.0553106f, -1.06183848f, 1.62139136f),
//            cv::Point3f(-0.17939515f, -1.04415142f, 1.62489016f),
//            cv::Point3f(0.16526156f, -0.70174933f, 1.60604109f),
//            cv::Point3f(-0.24733695f, -0.7025244f, 1.60869439f),
//            cv::Point3f(0.22571368f, -0.38324139f, 1.60380054f),
//            cv::Point3f(-0.31868614f, -0.38522304f, 1.5989569f),
//            cv::Point3f(0.12559686f, -0.32873257f, 1.58198847f),
//            cv::Point3f(-0.44751469f, -0.13163584f, 1.56567303f),
//            cv::Point3f(0.00448063f, -0.04716448f, 1.57395222f),
//            cv::Point3f(-0.20554063f, -0.048364f, 1.56829825f),
//            cv::Point3f(-0.0031295f, 0.3368551f, 1.55145973f),
//            cv::Point3f(-0.18427243f, 0.29841277f, 1.56373082f),
//            cv::Point3f(-0.00495475f, 0.6422929f, 1.62380712f),
//            cv::Point3f(-0.15420029f, 0.57830318f, 1.63110222f),
//            cv::Point3f(-0.04407379f, 0.80685279f, 1.62371142f),
//            cv::Point3f(0.01878638f, 0.79056144f, 1.57397098f),
//            cv::Point3f(-0.00459393f, 0.66552252f, 1.63903182f),
//            cv::Point3f(-0.26810113f, 0.69632733f, 1.47698203f),
//            cv::Point3f(-0.28558533f, 0.67574139f, 1.64089952f),
//            cv::Point3f(-0.11460012f, 0.61647446f, 1.65084556f),
//            cv::Point3f(-0.17995951f, -1.08626549f, 1.60109917f)
//    };
//    mJointsPose.clear();
//    for(const auto &i:result){
//        mJointsPose.push_back(glm::translate(glm::mat4(1.0f),{i.x,-i.y,-i.z}));
//    }
    //====================================
    for(const auto &model:mJointsPose){
        CubeRender::Cube cube{};
        cube.model=model;
        cube.scale=0.01f;
        cubes.push_back(cube);
    }
//    if(pp) infof("RT: %s",GlmMat4_to_String(MarkerDetector::GetTransMatFromRT({0,0,0},{0,0,0},glm::mat4(1.0f))).c_str())
//    if(pp)mCubeRender->render(project,MarkerDetector::GetTransMatFromRT({0,0,0},{0,0,0},glm::mat4(1.0f)),cubes);
    if(vmat_ptr){
//        mCubeRender->render(project,MarkerDetector::GetTransMatFromRT({0,0,0},{0,0,0},CV_Matx44f_to_GLM_Mat4(*vmat_ptr)),cubes);
        mCubeRender->render(project,view,cubes);
    }
    mResultMutex.unlock();
}
void HumanDetector::thread_main(const std::string &ip,int port){
    try{
        std::unique_ptr<ff::NetcallServer> serv=std::make_unique<ff::NetcallServer>(ip.c_str(),port);
        mServerIsStarting.store(false);
        mServerStarted.store(true);
        while(true){
            if(mThreadStop.load()) break;
            if(mImageProcessed.load()) continue;
            mImageMutex.lock();
            if(!mImage.empty()&&mCameraMatValid.load()){
                ff::NetObjs objs={{"cmd",  "estpose"},
                                  {"image",ff::nct::Image(mImage.clone(),".jpg")},
//                                  {"image",ff::nct::Image(cv::imread(Config::AppDataDir+"/Image/2Markers.png"),".jpg")},
                                  {"camK", mCameraMat},
                                  };
                auto time_cost=CurrentMSecsSinceEpoch();
                auto robjs=serv->call(objs);
                time_cost=CurrentMSecsSinceEpoch()-time_cost;
                infof("Server Call: %lld ms",time_cost)
                if(!robjs.hasError()){
                    auto state=robjs["n_objs"].get<std::string>();
                    if(std::stoi(state)==1){
                        auto result=robjs["points3d"].getv<cv::Point3d>();
                        std::stringstream _ss;
                        _ss<<result;
                        infof("Get Result: %s",_ss.str().c_str())
                        //=========== Set Result ===========
                        mResultMutex.lock();
                        mJointsPose.clear();
                        for(const auto &i:result){
                ...
                else warnf("robjs has error")
                mImageProcessed.store(true);
            }
            mImageMutex.unlock();
        }
    }
    catch(const std::exception &e){
        errorf("Connect to Detect Server Failed: %s",e.what())
    }
    catch(...){
    }
}
