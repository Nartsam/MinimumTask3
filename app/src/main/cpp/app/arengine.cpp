...
int ARInputs::Update(AppData &appData, SceneData &sceneData, FrameDataPtr frameDataPtr)
{
    ...
    if(!frameData.img.empty())
    {
        ...
        //================= For CameraTracking ===================
        frameDataPtr->imgColor = img.clone();
        frameDataPtr->imgDepth = img.clone();//cv::Mat();
        frameDataPtr->timestamp=(double)frameData.timestamp/1000.0;
        sceneData.getMainCamera().ARSelfPose=frameData.cameraMat;
        sceneData.getMainCamera().transform.setPose(frameData.cameraMat);
        //========================================================
    }
