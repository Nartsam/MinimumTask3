...
struct OpenXrProgram : IOpenXrProgram {
    ...
    static void OnCameraUpdate(const uint8_t* data, uint32_t size, uint32_t width, uint32_t height, uint64_t timestamp) {
        ...
        //执行自行添加的回调函数
        if(CameraUpdateCallbackListMutex.try_lock()){
            for(const auto &[key,func]:CameraUpdateCallbackList) func(image,m,timestamp);
            CameraUpdateCallbackListMutex.unlock();
        }
        else warnf("回调函数列表CameraUpdateCallbackList正在被另一个线程使用，此次未执行回调函数")
    }
...
//        };
    ...
    void CreateWorldSpaces() {  //新增: 仿照CreateVisualizedSpaces函数，创建m_WorldSpace的XR世界空间
        CHECK(m_session != XR_NULL_HANDLE);
        XrReferenceSpaceCreateInfo referenceSpaceCreateInfo{XR_TYPE_REFERENCE_SPACE_CREATE_INFO};
        referenceSpaceCreateInfo.poseInReferenceSpace = Math::Pose::Identity();
        referenceSpaceCreateInfo.referenceSpaceType = XR_REFERENCE_SPACE_TYPE_LOCAL; // 或 XR_REFERENCE_SPACE_TYPE_STAGE
        CHECK_XRCMD(xrCreateReferenceSpace(m_session, &referenceSpaceCreateInfo, &m_WorldSpace));
    }
...
//                infof(("========== Pressed: "+std::string(i.second)).c_str());
        ...
        { // 控制器侧边MENU键
            ...
            if ((menuValue.changedSinceLastSync == XR_TRUE) || (menuValue.currentState == XR_TRUE)) {
                if (pfnXrRecenterPhonePose != nullptr) { //--重置3Dof射线
                    Log::Write(Log::Level::Info, Fmt("Reset Controller Ray"));
...
//                                                     spaceLocation.pose.orientation.x, spaceLocation.pose.orientation.y, spaceLocation.pose.orientation.z, spaceLocation.pose.orientation.w));
                    if(OPENXR_PROGRAM_USE_HAND_AIM) m_application->setControllerPose(int(hand), spaceLocation.pose);
//#ifdef USE_HAND_AIM
...
//                                                     spaceLocation.pose.position.x, spaceLocation.pose.position.y, spaceLocation.pose.position.z,
//                                                     spaceLocation.pose.orientation.x, spaceLocation.pose.orientation.y, spaceLocation.pose.orientation.z, spaceLocation.pose.orientation.w));
                    if(!OPENXR_PROGRAM_USE_HAND_AIM) m_application->setControllerPose(int(Side::RIGHT), spaceLocation.pose);  //2025-10-23 这里的leftright参数默认是0,也就是将控制器视为左手控制
//#ifndef USE_HAND_AIM
//                    m_application->setControllerPose(int(Side::RIGHT), spaceLocation.pose);  //2025-10-23 这里的leftright参数默认是0,也就是将控制器视为左手控制
//#endif
                }
            }
        }
        ...
        //=============================================================================
        auto worldJointPos=&jointLocations;
//        XrHandJointLocationEXT worldJointPos[Side::COUNT][XR_HAND_JOINT_COUNT_EXT];
        m_application->setHandJointWorldLocation((XrHandJointLocationEXT*)worldJointPos);
        //=============================================================================
    ...
    private:
        ...
        XrSpace m_WorldSpace{XR_NULL_HANDLE};  //新增: XR世界空间
