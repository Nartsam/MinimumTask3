...
void Cylinder::initialize() {
    ...
    for (int i = 0; i <= mSegments; ++i) {
        ...
        float u = static_cast<float>(i) / mSegments;
    ...
    // 顶盖和底盖中心点 (用 0.5,0.5 作为UV中心)
    int bottomCenterIndex = mVertices.size() / 5;
    mVertices.insert(mVertices.end(), {0.0f,0.0f,-halfH, 0.5f,0.5f});

    int topCenterIndex = mVertices.size() / 5;
    mVertices.insert(mVertices.end(), {0.0f,0.0f,+halfH, 0.5f,0.5f});
