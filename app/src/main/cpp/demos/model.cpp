...
void Model::initShader() {
    ...
    if (init) {
        ...
        const char* vertexShaderCode = R"_(
            ...
            uniform mat3 normalMatrix; // 新增：由 CPU 计算并上传
            ...
            void main(){
                ...
                if (!has_bone) total_position = vec4(aPos, 1.0f);
        ...
        const char* fragmentShaderCode = R"_(
            ...
            uniform vec3 viewPos;     // 相机位置 (由 C++ 设置)
            uniform vec3 lightColor;  // 光颜色
            uniform vec3 diffuseColor; // 无纹理时使用的默认颜色
            void main(){
...
//    infof("model:%s, scene:%s, mNumMeshes:%d, mNumMaterials:%d, mNumAnimations:%d, mNumTextures:%d", modelFileName.c_str(),scene->mName.C_Str(), scene->mNumMeshes, scene->mNumMaterials, scene->mNumAnimations, scene->mNumTextures);
    ...
    initializeBoneNode();
    _valid=true; // ** Added in 2025-07-04 **
    return true;
}
void Model::draw() {
...
bool Model::render(const glm::mat4& p, const glm::mat4& v, const glm::mat4& m) {
    ...
    // 计算 normalMatrix 并上传（注意 glm::mat3 从 glm::mat4 截取左上3x3）
    glm::mat3 normalMat = glm::transpose(glm::inverse(glm::mat3(m)));
    mShader.setUniformMat3("normalMatrix", normalMat);
    // 光源位置（AR 眼镜中你可以用固定光）
    mShader.setUniformVec3("lightPos", 0.0f, 2.0f, 2.0f);
    // 相机位置（从 view 矩阵反求）
    glm::vec3 camPos = glm::vec3(glm::inverse(v)[3]);
    mShader.setUniformVec3("viewPos", camPos);
    // 光的颜色
    mShader.setUniformVec3("lightColor", 1.0f, 1.0f, 1.0f);
    // 默认颜色（无纹理使用）
    mShader.setUniformVec3("diffuseColor", 1.0f, 1.0f, 1.0f);
    draw();
    glUseProgram(0);
    return true;
}
...
bool Model::loadLocalModel(const std::string &modelFileName,bool normalize,const aiScene* external_scene) {
    initShader();

    Assimp::Importer importer;
    const aiScene* scene=external_scene;
    if(scene==nullptr){
        scene=importer.ReadFile(modelFileName, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_CalcTangentSpace);
        //const aiScene* scene = importer.ReadFileFromMemory(fileData.data(), fileData.size(), aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_CalcTangentSpace);
        if (scene == nullptr || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || scene->mRootNode == nullptr ) {
...
//    for(auto i=texture_filename_list.size();i<mesh_name_list.size();++i){ //如果贴图数量少于mesh数量,就把剩余的mesh使用纯白色贴图,避免渲染出来看不见的情况
//        bindMeshTexture(mesh_name_list[i],"hand/0.png");
//    }
    infof("model:%s, scene:%s, mNumMeshes:%d, mNumMaterials:%d, mNumAnimations:%d, mNumTextures:%d", modelFileName.c_str(),scene->mName.C_Str(), scene->mNumMeshes, scene->mNumMaterials, scene->mNumAnimations, scene->mNumTextures);
    processNode(scene->mRootNode, scene);
    //=========================== 把模型标准化，防止过大或过小 ================================
    if(normalize){
        ...
        for(auto &mesh:mMeshes){
            for(auto &v:mesh.second.mVertices){
                auto &pos=v.Position;
                pos.x-=(float)(sum_x/count); pos.y-=(float)(sum_y/count); pos.z-=(float)(sum_z/count);
    ...
    for(auto i=texture_filename_list.size();i<mesh_name_list.size();++i){ //如果贴图数量少于mesh数量,就把剩余的mesh使用纯白色贴图,避免渲染出来看不见的情况
        activeMeshTexture(mesh_name_list[i],"hand/0.png");
    }
    _valid=true;
    return true;
}
...
//=================================================================================================
Model Model::GetModelFromLocalFile(const std::string &model_name,const std::string &file_path,bool normalize){
    Model model(model_name);
    model.initialize();
    if(!file_path.empty()){
        if(model.loadLocalModel(file_path,normalize)){
            infof("Load Local Model Success: %s",file_path.c_str());
        }
        else errorf("Load Local Model Failed: %s",file_path.c_str());
    }
    return model;
}
