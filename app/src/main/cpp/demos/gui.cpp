...
//    // 1. 计算观察矩阵的逆矩阵（相机坐标系到世界坐标系）
//    glm::mat4 cameraInverse = glm::inverse(v);
//    // 2. 创建一个变换矩阵，将 GUI 放置在相机前方
//    glm::mat4 uiTranslation = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -3.0f));
//
//    // 3. 创建一个旋转矩阵，调整 GUI 的方向（可选）
//    // glm::mat4 uiRotation = glm::rotate(glm::mat4(1.0f), glm::radians(45.0f), glm::vec3(0.0f, 1.0f, 0.0f)); // 绕 Y 轴旋转 45 度
//
//    // 4. 计算最终的模型矩阵
//    glm::mat4 model = cameraInverse * uiTranslation; // * uiRotation;
...
void Gui::render(const glm::mat4& p, const glm::mat4& v,const glm::mat4& fix_on_view)  {
...
//    mShader.setUniformMat4("model", mModel);
    ...
    glm::mat4 cameraInverse = glm::inverse(v);
    mShader.setUniformMat4("model", cameraInverse * fix_on_view);
...
bool Gui::isIntersectWithLine(const glm::vec3& linePoint, const glm::vec3& lineDirection,bool update_event) {
    ...
    if (point.x >= planePoint2.x && point.x <= planePoint1.x 
     && point.y >= planePoint2.y && point.y <= planePoint1.y) {
        ...
        */
        ...
        if(update_event) updateMousePosition(posx, posy);
